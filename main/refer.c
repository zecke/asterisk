...
int ast_refer_notify_transfer_request(struct ast_channel *source, const char *referred_by,
				const char *exten, const char *replace, struct ast_channel *dest)
{
	...
	transfer_message = ast_ari_transfer_message_create(source, referred_by, exten, replace, dest);
	if (!transfer_message) {
		return -1;
	}
	...
	if (source_bridge) {
		...
		transfer_message->source_bridge = ast_bridge_snapshot_create(source_bridge);
		peer = ast_bridge_peer_nolock(source_bridge, source);
		if (peer) {
			ast_channel_lock(peer);
			transfer_message->source_peer = ast_channel_snapshot_get_latest(ast_channel_uniqueid(peer));
